...
if (!publisherName || publisherName === "$tfx_publisher") {
    publisherName = vssExtensions.publisher;
}
if (!extensionId || extensionId === "$tfx_extension_id") {
    extensionId = vssExtensions.id;
}
...
vssExtensions.id = extensionId;
vssExtensions.publisher = publisherName;
...
function replaceString(str, searchTerm, replaceTerm) {
    ...
    if (pointPos > -1) {
        str = str.replace(searchTerm, `.${extensionId}`);
